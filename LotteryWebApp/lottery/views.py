...
def add_draw():
    ...
    submitted_draw = submitted_draw.strip()

    if not submitted_draw:
        flash('Draw blank, not submitted.')
    else:
...
def check_draws():
    ...
    if len(played_draws) != 0:
        ...
        return render_template('lottery.html', results=played_draws, draw_key=draw_key)
...
@requires_roles('user')
def play_again():
    delete_played = Draw.__table__.delete().where(current_user.Draw.played)
