...
def decrypt(data, draw_key):
    return str(Fernet(draw_key).decrypt(data), 'utf-8')
...
class Draw(db.Model):
    ...
    # decrypting draw so it can be viewed
    def view_draw(self, draw_key):
        self.draw = decrypt(self.draw, draw_key)

    def draw_unencrypted(self):
        this_user = User.query.filter_by(id=self.user_id).first()
        return decrypt(self.draw, this_user.draw_key)
